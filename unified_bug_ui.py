...
class Robot:
    ...
    def point_in_polygon(self, x, y, polygon, buffer=0.5):
        """Ray casting algorithm for point-in-polygon with buffer"""
        inside = False
        n = len(polygon)
        px, py = x, y
        for i in range(n):
            x1, y1 = polygon[i]
            x2, y2 = polygon[(i + 1) % n]
            # Expand polygon edge by buffer
            if ((y1 > py) != (y2 > py)) and (
                px < (x2 - x1) * (py - y1) / (y2 - y1 + 1e-10) + x1 + buffer):
                inside = not inside
        return inside

    def segment_intersects_polygon(self, p1, p2, polygon, buffer=0.5):
        """Check if segment p1-p2 intersects polygon edges (with buffer)"""
        n = len(polygon)
        for i in range(n):
            q1 = polygon[i]
            q2 = polygon[(i + 1) % n]
    ...
    def segment_collision_with_obstacles(self, p1, p2, obstacles, buffer=0.5):
        """Check if segment p1-p2 collides with any obstacle (with buffer)"""
        for obstacle in obstacles:
    ...
    def scan_environment(self, obstacles, canvas=None):
        """Scan the environment using LIDAR and optionally visualize"""
        self.lidar_data = np.ones(self.lidar_resolution) * self.lidar_range
        lidar_endpoints = []
        
        for i, angle in enumerate(self.lidar_angles):
            # Calculate ray direction
            dx = math.cos(angle + self.orientation)
            dy = math.sin(angle + self.orientation)
            ...
            for obstacle in obstacles:
                for j in range(len(obstacle)):
                    ...
                    x1, y1 = self.x, self.y
                    x2, y2 = self.x + dx * self.lidar_range, self.y + dy * self.lidar_range
                    x3, y3 = p1
                    x4, y4 = p2
            ...
            self.lidar_data[i] = min_dist
            endpoint = (self.x + dx * min_dist, self.y + dy * min_dist)
...
def generate_obstacles(num_obstacles, grid_size):
    """Generate random polygon obstacles"""
    obstacles = []
    min_vertices = 3
    max_vertices = 6
...
def main():
    ...
    def run_comparison():
        ...
        for algo, func in algo_list:
            path = func(start[0], start[1], goal[0], goal[1], obstacles, GRID_SIZE, canvas)
            # Draw the path
            for i in range(len(path) - 1):
                x1, y1 = path[i]
                x2, y2 = path[i + 1]
            ...
            for i in range(len(path) - 1):
                ...
                segment_length = math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)
                path_length += segment_length
            results.append((algo, path_length, len(path)))
        # Show comparison table
        comparison_text = "Algorithm Comparison:\n\n"
        comparison_text += f"{'Algorithm':12} | {'Length':>8} | {'Waypoints':>9}\n"
        comparison_text += "-"*34 + "\n"
        for algo, length, waypoints in results:
            comparison_text += f"{algo:12} | {length:8.2f} | {waypoints:9d}\n"
    ...
    caption = f"Fig. 1. HD-Bug path planning with {OBSTACLE_COUNT} obstacles. " \
